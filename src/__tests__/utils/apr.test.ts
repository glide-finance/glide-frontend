...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { glideRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null)
    expect(glideRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { glideRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '')
    expect(glideRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { glideRewardsApr, lpRewardsApr } = getFarmApr(BIG_TEN, new BigNumber(1), new BigNumber(100000), '')
    expect(glideRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
